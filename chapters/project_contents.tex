...
Jetbrains also had a plugin for original Git-Time-Metric, but the IDE has gone through lots of changes since the last update to the plugin.
...
\begin{enumerate}
    \item Opening the file in editor.
...
In response, plugin gets the time since the last commit (uncommitted time), which it shows on the bottom status bar.
...
All smaller level features can be included while using higher level option.
...
After that you need to search for the repository from web interface and click \textit{"Start tracking"} button.
...
Manually editing configuration is meant for developer as it gives more control over application and REST API is
...
It also allows adding automatic data collection to user repositories more easily, as user can browse through
...
Requirements for grouping system were:
\begin{itemize}
    \item It shall be possible to group both repositories and groups already consisting of repositories into a bigger groups.
...
Single user can belong to multiple roles and also new roles can be dynamically added to database as they are
...
In that case user has to log in and head to the profile page where he can find link accounts tab.
...
User can start tracking a new repository by clicking start tracking and then adding a webhook to the according OAuth platform.
If start tracking is clicked and everything is successful, a blue info box with instructions for webhook setup will pop up.
In case user wants to start tracking again then the repository can be found at the end of the list with a button initialize again.
...
From the sidebar the user can navigate to the necessary page and on top of the page user has selection of properties according to the graphs.
...
The first graph is a combination of line and bar charts which describes time spent writing code in a period of time and also shows how many users were active at the interval.
...
The second graph is line chart which describes activity hours during the chosen period.
...
The third graph is a bar chart which describes time spent in folders or files.
X-axis shows the time period and Y-axis shows the time spent during day/week/month depending on the chosen interval.
...
At the end of the page there is export data button which allows downloading Comma Separated Values (CSV) file with data.
The data is filtered by the same inputs that filter the data in tables but is grouped
by also commits giving higher resolution that the tables described above.
