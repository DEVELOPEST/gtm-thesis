We decided to base our application on~\nameref{subsec:git-time-metric} and have time tracking data stored in Git Notes
...
\begin{enumerate}
    ...
    \item \textbf{Applications that are responsible for analysing and displaying data.} These are Frontend and Backend app that are installed on any server.
\end{enumerate}
...
\end{enumerate}
Design patterns such as graph sizes, colours, spacings, fonts etc. were chosen in such a way that it would look similar to CoreUI graphs.
