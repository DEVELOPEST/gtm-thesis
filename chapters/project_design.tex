...
In our case frontend is a website where users can interact with our backend.
Users can register/login with username/password or with OAuth.
They can see their data from graphs.

Frontend has been built with TypeScript using React JS library.
React is Javascript library for creating user interfaces.
Choosing correct frontend framework/library is not as important as choosing correct backend programming language and framework since all the main frameworks/libraries (Angular, React, Vue, Aurelia, etc) are very good.
Choosing the correct framework/library comes more down to personal preference.
We chose React JS because it is ligthter than others and it gives more freedom.

Therefore choosing TypeScript over JavaScript was not just personal preference.
TypeScript is nothing else than JavaScript.
TypeScript code is converted into JavaScript but it is strongly-typed language which is very important feature to keep code organized and easy to read.
This decision was not made in the beginning of this project but since the code kept growing and growing we understood that strongly-typed language is needed to keep everything organized so we decided to convert everything to TypeScript.

State management is another area that does not have a certain winner.
Choosing a state management library comes again down to personal preference.
